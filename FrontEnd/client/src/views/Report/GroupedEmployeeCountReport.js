...
export function GroupedEmployeesReport() {
  ...
  useEffect(() => {
    let token = sessionStorage.getItem("token");
    Axios.post(
      "http://localhost:3001/report/create_grouped_employee_report",
      formValues,
      { headers: { Authorization: `Bearer ${token}` } }
    )
      .then((response) => {
  ...
  return (
    <div className="background-Report">
      <div>
        <h1 class="title">
          Employee Count Grouped by {formValues.parameter} Report
        </h1>
            ...
            {rows.map((row) => {
              ...
              return (
                <tr {...row.getRowProps()}>
                  {row.cells.map((cell) => {
                    if (
                      parameter === cell.column.id ||
                      cell.column.id === "Employee Count"
                    ) {
                      return (
